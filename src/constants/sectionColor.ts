export const sectionTextColors: Record<string, string> = {
  home: 'text-green-500',
  about: 'text-blue-500',
  experience: 'text-orange-500',
  skills: 'text-purple-500',
  projects: 'text-sky-500',
  github: 'text-violet-500',
  education: 'text-pink-500',
  certifications: 'text-lime-500',
  contact: 'text-red-500',
}

export const sectionGradientColors: Record<string, string> = {
  home: 'from-green-500 via-emerald-400 to-green-500',
  about: 'from-blue-500 via-cyan-400 to-blue-500',
  experience: 'from-orange-500 via-amber-400 to-orange-500',
  skills: 'from-purple-500 via-fuchsia-400 to-purple-500',
  projects: 'from-sky-500 via-blue-400 to-sky-500',
  github: 'from-violet-500 via-indigo-400 to-violet-500',
  education: 'from-pink-500 via-rose-400 to-pink-500',
  certifications: 'from-lime-500 via-green-400 to-lime-500',
  contact: 'from-red-500 via-rose-400 to-red-500',
}

export const sectionGradientBackgrounds: Record<string, string> = {
  home: 'linear-gradient(120deg, #22c55e, #34d399, #22c55e)',
  about: 'linear-gradient(120deg, #3b82f6, #22d3ee, #3b82f6)',
  experience: 'linear-gradient(120deg, #f97316, #fbbf24, #f97316)',
  skills: 'linear-gradient(120deg, #a855f7, #e879f9, #a855f7)',
  projects: 'linear-gradient(120deg, #0ea5e9, #60a5fa, #0ea5e9)',
  github: 'linear-gradient(120deg, #8b5cf6, #6366f1, #8b5cf6)',
  education: 'linear-gradient(120deg, #ec4899, #fb7185, #ec4899)',
  certifications: 'linear-gradient(120deg, #84cc16, #4ade80, #84cc16)',
  contact: 'linear-gradient(120deg, #ef4444, #fb7185, #ef4444)',
}
